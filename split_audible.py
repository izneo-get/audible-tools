...
if len(sys.argv) < 2:
    ...
    print(
        sys.argv[0]
        + ' <CODE | META_DATA_URL | JSON_FILE> <FILE_IN> ["ffmpeg options" [OUTPUT_EXT [CHAPTER_NO_OFFSET]]]'
    )
    print()
    print("Examples:")
    ...
    print(
        sys.argv[0]
        + ' "https://stories.audible.com/audibleapi/1.0/content/B00TKSFFJE/metadata?drm_type=Hls&response_groups=chapter_info"'
    )
...
chapter_offset = 0
if len(sys.argv) > 5:
    chapter_offset = sys.argv[5]

if os.path.exists(chapters_url):
    with open(chapters_url, "r") as f:
        content = json.load(f)
else:
...
for i, chapter in enumerate(chapters):
    chap_num = i + chapter_offset
    start_ms = (
        str(time.strftime("%H:%M:%S", time.gmtime(chapter["start_offset_ms"] / 1000)))
        + "."
        + f"{(chapter['start_offset_ms'] % 1000):03d}"
    )
    length_ms = (
        str(time.strftime("%H:%M:%S", time.gmtime(chapter["length_ms"] / 1000)))
        + "."
        + f"{(chapter['length_ms'] % 1000):03d}"
    )
